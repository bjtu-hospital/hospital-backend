...
**状态说明**:
- 验证通过后，Redis 写入 `sms:verified:{phone}` 标记（TTL=15分钟）
- 此标记用于注册接口的前置校验
- 成功注册后自动消费该标记（一次性使用）
...
从 2025-12-09 起，所有预约订单新增 `sourceType` 字段，用于标识预约的来源：

- **`normal`**: 普通预约（用户直接挂号创建）
- **`waitlist`**: 候补订单（通过候补队列创建，包括未转预约和已转预约的候补订单）

**应用场景**：
1. 前端可根据 `sourceType` 显示不同的标签（如"候补"或"候补成功"）
2. 数据分析可统计候补转化率
3. 支持针对不同来源的订单实施差异化策略

**设置规则**：
- 普通预约（`POST /appointments`）创建时设为 `normal`
- 候补订单（`POST /patient/waitlist`）创建时设为 `waitlist`，转预约后保持不变

**涉及接口**：
- `GET /appointments` - 获取我的预约列表
- `GET /patient/my-initiated-appointments` - 获取我创建的订单列表
- `GET /appointments/{appointmentId}` - 获取预约详情
- `POST /patient/waitlist` - 加入候补队列（设置为 waitlist）
- `POST /patient/waitlist/{waitlistId}/convert` - 候补转预约（保持 waitlist）
...
- `canReschedule`: 是否可改约（基于排班开始时间判断）
- `sourceType`: 预约来源，"normal"(普通预约) 或 "waitlist"(候补转预约)
- `queueNumber`: 排队号码（预约阶段返回 null，就诊时由就诊系统动态分配）
...
5. **source_type 标记**：候补订单创建时 `source_type` 即被设置为 `"waitlist"`，用于区分普通预约和候补渠道。
...
4. 状态更新：
   ...
   - **source_type 保持为 `waitlist`**（保留候补渠道标记）
   - 剩余号源 remaining_slots - 1
5. 支付超时：返回 `expiresAt`（默认 30 分钟后）供前端倒计时。

**重要提示**: 候补订单从创建时 `sourceType` 就是 `"waitlist"`，转预约后保持不变，用于前端识别并显示"候补成功"标签。
...
**响应字段说明**:
- `sourceType`: 预约来源标识，候补转预约固定为 `"waitlist"`，用于前端识别并显示"候补成功"标签
...
## 6. 支付预约订单 Post: `/appointments/{appointmentId}/pay`

支付指定的预约订单。支持银行卡、支付宝、微信三种支付方式。
...
字段说明：
- `method`: 支付方式，必填，可选值: `bank`(银行卡) / `alipay`(支付宝) / `wechat`(微信)
- `remark`: 支付备注，可选

### 支付规则:
1. 仅 PENDING 状态且 payment_status 为 PENDING 的订单可支付
2. 订单金额必须有效（price > 0）
3. 支付成功后订单状态变更为 CONFIRMED
4. 当前为模拟支付，直接标记为已支付

### 请求示例:
```
POST /appointments/1001/pay
Content-Type: application/json
...
```json
{
    ...
    "message": {
        "success": true,
        "orderId": 1001,
        "orderNo": "2025120812345678",
        "paymentStatus": "paid",
        "paymentTime": "2025-12-08 14:30:00",
        "method": "alipay",
        "amount": 60.00
    }
}
